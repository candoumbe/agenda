...
ADR-001 is accepted and imposes Keycloak as the identity provider. ADR-002 selected angular-auth-oidc-client for the Angular frontend and identified unresolved questions that impact security, operability, and future evolution.
...
Answer: ADR-001 fixes provider choice, but code-level coupling should still be constrained to reduce migration risk.
...
Recommendation: Define a canonical AuthzMap document (role -> capability), map Keycloak realm/client roles to normalized app capabilities in one transformer, and make guards/components depend only on capabilities.
...
Recommendation: Keep access token in memory only. If refresh token in browser is unavoidable, use sessionStorage with strict CSP and short lifetimes. Do not use localStorage for bearer tokens.
...
Recommendation: Minimum suite: unit tests for auth adapter and claim mapping, integration tests for guard/interceptor behavior, and E2E tests for login, token renewal failure, and logout flow. Add contract checks for expected claims used by UI authorization.

10. Better Auth reevaluation criteria
Answer: Re-platforming auth without objective triggers creates churn.
Recommendation: Reevaluate Better Auth only if two or more conditions hold for one quarter: multi-frontend session orchestration pain, repeated token-handling incidents, need for centralized social/provider brokering beyond Keycloak defaults, or measurable delivery slowdown caused by SPA-direct auth complexity.
...
Recommendation: Adopt a weighted matrix with these weights: Security 30%, Operational complexity 20%, Delivery speed 15%, Maintainability 15%, User experience 10%, Portability 10%. Require option scoring evidence and architecture review sign-off before changing auth direction.

## Rationale

This decision keeps strict consistency with ADR-001 by retaining Keycloak as the imposed IdP while reducing ambiguity from ADR-002 through explicit policy-level answers.
...
- Any proposal to add BFF/Better Auth must include matrix scoring and trigger evidence from response 10 and 12.
