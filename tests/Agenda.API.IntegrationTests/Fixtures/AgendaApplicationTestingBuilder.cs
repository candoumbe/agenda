...
public class AgendaApplicationTestingBuilder : IAsyncDisposable
{
    ...
    /// </remarks>
    public async Task<DistributedApplication> StartAsync(TimeSpan startStopTimeout, CancellationToken cancellationToken = default)
    {
        _app  = await _sutBuilder.BuildAsync(cancellationToken).WaitAsync(cancellationToken);

        DistributedApplicationModel applicationModel = _app.Services.GetRequiredService<DistributedApplicationModel>();
