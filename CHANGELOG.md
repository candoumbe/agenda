...
- Added `GET /health` readiness endpoint (development only) : returns `200 OK` when all health checks (including database connectivity) pass, otherwise `503 Service Unavailable`.
- Added `GET /alive` liveness endpoint (development only) : returns `200 OK` when the process is up and responsive.
