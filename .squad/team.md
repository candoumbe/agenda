# Squad Team

> agenda

## Coordinator

| Name | Role | Notes |
|------|------|-------|
| Squad | Coordinator | Routes work, enforces handoffs and reviewer gates. |

## Members

| Name | Role | Charter | Status |
|------|------|---------|--------|
| Ripley | Lead | .squad/agents/ripley/charter.md | Active |
| Ash | Architect | .squad/agents/ash/charter.md | Active |
| Dallas | Frontend Dev | .squad/agents/dallas/charter.md | Active |
| Bishop | Backend Dev | .squad/agents/bishop/charter.md | Active |
| Hicks | Tester | .squad/agents/hicks/charter.md | Active |
| Vasquez | Security | .squad/agents/vasquez/charter.md | Active |
| Lambert | Docs / DevRel | .squad/agents/lambert/charter.md | Active |
| Scribe | Session Logger | .squad/agents/scribe/charter.md | Active |
| Ralph | Work Monitor | .squad/agents/ralph/charter.md | Active |

## Project Context

- **Project:** agenda
- **Created:** 2026-04-23
- **Requested By:** vscode
- **Stack:** .NET backend microservice, PostgreSQL/SQLite datastore, Angular microfrontend
- **Architecture:** Vertical Slice
